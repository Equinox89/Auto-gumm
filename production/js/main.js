...
;(function ($, window, document, undefined) {
    ...
    var headerTopPos = void 0,
        headerVisibleHeight = void 0;
    ...
    $(document).ready(function () {
        ...
        $('.tab-menu').tab({
            change: function change(e, tab) {
                var slickBig = $(tab).find('.details__slider-master'),
        ...
        $document.scroll(function () {
            ...
            headerTopPos = $header.offset().top - $window.scrollTop() < 0 ? $header.offset().top - $window.scrollTop() : 0;
        ...
        $document.on('click', '.mobile-menu__back', function () {
            var self = $('nav li.is-visible:last');
                ...
                callbacks: {
                    elementParse: function elementParse(item) {
                        if (item.el[0].classList.contains('video-popup')) {
                            ...
                            item.type = 'image', item.tLoading = 'Loading image #%curr%...', item.image = {
        ...
        function findVideos() {
            $(document).on('click', '.video__button', function () {
                ...
                var block = $(this).closest('.video__inner');
                var url = block.find('.video__media').attr('data-embed');

                var iframe = $('<iframe allowfullscreen="" allow="autoplay" class="video__media"></iframe>');
                var source = url.replace('watch?v=', 'embed/');
...
(function ($) {
    ...
    var rating = function () {
        ...
        Rating.prototype.initRating = function (rating) {
            ...
            var fullItem = this.$el.find('.rating__item--full');

            if (!Number.isInteger(value)) {
                var result = Math.asin(2 * (value - Math.trunc(value)) - 1) / Math.PI + 0.5;
                fullItem.last().next().addClass(this.options.fractionClass).attr('style', '--fraction: ' + result.toFixed(2));
            }
        };
...
(function ($) {
    ...
    var tab = function () {
        ...
        Tab.prototype.initBox = function (target) {
            var isAuto = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
            ...
            if (thisTab.length > 0) {
                ...
                var $swiper = thisTab.find('.swiper-container:not(.swiper-container-initialized)');
                if ($swiper) {
                    SwiperModule.init($swiper);
                }
        ...
        Tab.prototype.initTab = function () {
            var item = this.$el.find('.' + this.options.itemDefault).addClass(this.options.itemActive);
...
var SwiperModule = function () {
    ...
    var initSwiper = function initSwiper(elem) {
        if (!elem || !elem.length) {
            return;
        }

        var key = elem.data(dataParamsName);
        if (!key) {
            return;
        }
